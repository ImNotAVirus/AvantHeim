defmodule GameService.Algorithms.ExperienceAlgorithms do
  @moduledoc """
  TODO: Documentation
  """
  ...
  for {constant, level} <- Enum.with_index(@xp_constants, 1) do
  ...
  @job_exp_constants List.flatten([
  ...
  for {constant, level} <- Enum.with_index(@job_exp_constants, 1) do
  ...
  @hero_exp_constants List.flatten([
  ...
  for {constant, level} <- Enum.with_index(@hero_exp_constants, 1) do
  ...
  @specialist_exp_constants List.flatten([
  ...
  for {constant, level} <- Enum.with_index(@specialist_exp_constants, 1) do
  ...
  @fairy_max_level Application.compile_env(:game_service, :fairy_max_level, 80)
