defmodule LoginService.Endpoint.Cryptography do
  ...
  """
  @spec next(binary(), map(), binary()) :: {binary(), binary()}
  def next(packet, _assigns, acc \\ <<>>)

  def next(<<>>, _assigns, acc) do
    {acc, <<>>}
  end

  def next(<<0xD8, rest::binary>>, _assigns, acc) do
    {acc, rest}
  end

  def next(<<c, rest::binary>>, assigns, acc) do
    next(rest, assigns, acc <> <<c>>)
  end
