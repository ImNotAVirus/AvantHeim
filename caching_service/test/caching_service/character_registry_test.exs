defmodule CachingService.CharacterRegistryTest do
  ...
  describe "write_character/1" do
    ...
    test "updates an existing Character", %{id: id} do
      ...
      assert [_] = get_character(id)
      assert {:ok, ^updated_character} = CharacterRegistry.write_character(updated_character)
      assert [cached_character] = get_character(id)
      assert attribute_index(cached_character, 2) == "ThisIsANewName"
    end
  end

  describe "get_character_by_id/1" do
    test "returns an existing character", %{id: id} do
      {:ok, character} = create_character(id)
      assert {:ok, ^character} = CharacterRegistry.get_character_by_id(id)
    end

    test "returns :not_found if not existing", %{id: id} do
      assert {:error, :not_found} = CharacterRegistry.get_character_by_id(id)
    end
  end
  ...
  describe "get_character_by_name/1" do
    test "returns an existing character", %{id: _id} do
      ...
      assert {:ok, ^character} = CharacterRegistry.get_character_by_name(character.name)
    end

    test "returns :not_found if not existing", %{id: _id} do
      assert {:error, :not_found} = CharacterRegistry.get_character_by_name("NonExisting")
    end
  end

  describe "delete_character_by_account_id/1" do
    test "deletes an existing character", %{id: id} do
      ...
      assert {:ok, ^character} = CharacterRegistry.delete_character_by_account_id(id)
      assert [] = get_character(character.id)
    end

    test "returns :not_found if not existing", %{id: id} do
      assert {:error, :not_found} = CharacterRegistry.delete_character_by_account_id(id)
    end
  end

  describe "get_characters_by_map_id/1,2" do
    test "returns an empty list when no character", %{id: id} do
      assert {:ok, []} = CharacterRegistry.get_characters_by_map_id(id)
    end

    test "returns existing characters", %{id: id} do
      {:ok, character} = create_character_with_map_id(id)
      assert {:ok, [^character]} = CharacterRegistry.get_characters_by_map_id(id)

      {:ok, character2} = create_character_with_map_id(id)
      assert {:ok, [_, _] = characters} = CharacterRegistry.get_characters_by_map_id(id)
    ...
    test "returns existing characters with guards", %{id: id} do
      ...
      assert {:ok, [^character]} =
               CharacterRegistry.get_characters_by_map_id(id, {:==, :id, character.id})

      assert {:ok, [^character2]} =
               CharacterRegistry.get_characters_by_map_id(id, {:!=, :id, character.id})

      assert {:ok, [^character2]} =
               CharacterRegistry.get_characters_by_map_id(id, {:==, :id, character2.id})
  ...
  defp create_character() do
    character_attrs_mock() |> CharacterRegistry.init_character()
  end

  defp create_character(id) do
    %{id: id} |> character_attrs_mock() |> CharacterRegistry.init_character()
  end

  defp create_character_with_acc_id(id) do
    %{account_id: id} |> character_attrs_mock() |> CharacterRegistry.init_character()
  end

  defp create_character_with_map_id(id) do
    %{map_vnum: id} |> character_attrs_mock() |> CharacterRegistry.init_character()
  end
